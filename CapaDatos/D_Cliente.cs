...
namespace CapaDatos
{
    public class D_Cliente
    {
        ...
        public DataTable SelectTextoCliente(string criterioDeBusqueda, int atributo)
        {
            SqlCommand command = new SqlCommand("sp_auxiliarCombo", DB);
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.AddWithValue("@idPuerto", atributo);
            SqlDataAdapter dataAdapter = new SqlDataAdapter(command);
            DataTable dataTable = new DataTable();
            dataAdapter.Fill(dataTable);
            return dataTable;
